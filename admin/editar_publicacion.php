...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["edit_publicacion"])) {
    ...
    $disponible_para_todos = isset($_POST['disponible_para_todos']) ? 1 : 0;
    ...
    if (empty($titulo)) {
        ...
        $sql = "UPDATE publicacion SET titulo = ?, descripcion = ?, tipo = ?, imagen_publicacion = ?, estado = ?, orden = ?, disponible_para_todos = ? WHERE id_publicacion = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("sssssiii", $titulo, $descripcion, $tipo, $imagen_publicacion, $estado, $orden, $disponible_para_todos, $id_publicacion);
            if ($stmt->execute()) {
...
<div class="container mt-4">
    ...
    <form action="editar_publicacion.php?id=<?php echo $id_publicacion; ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="form-check mb-3">
            <input class="form-check-input" type="checkbox" value="1" id="disponible_para_todos" name="disponible_para_todos" <?php if (!empty($publicacion['disponible_para_todos']) && $publicacion['disponible_para_todos'] == 1) echo 'checked'; ?>>
            <label class="form-check-label" for="disponible_para_todos">
                Disponible para todos (Público para cualquier usuario)
            </label>
        </div>
